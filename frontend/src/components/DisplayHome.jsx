      ...
      <div className="mb-4">
        ...
        <div className="flex gap-6 overflow-x-auto pb-4">
          {artists.map((item, index) => (
            <div
              key={index}
              className="min-w-[160px] bg-neutral-800 rounded-xl p-3 shadow-md hover:scale-105 transition-transform duration-300 cursor-pointer"
              onClick={()=>navigate(`/detailArtist/${item.id}`)}
            >
              <img
                src={item.image_url}
                alt={item.name}
              ...
              />
              <div className="mt-3">
                <h2 className="text-white font-semibold text-lg truncate text-center">
                  {item.name}
                </h2>
